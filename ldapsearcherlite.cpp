...
LdapSearcherLite::LdapSearcherLite(QObject *parent) : QObject(parent)
{

}

int LdapSearcherLite::LDInit(QString url)
{
    if(ld != Q_NULLPTR) {
...
int LdapSearcherLite::LDBind(QString dn, QString password)
{
    const char *adn;
...
int LdapSearcherLite::LDMailSearch(QString mail)
{
    int result = LDAP_SUCCESS;
    ...
    QString find = QString("(&(objectCategory=person)(mail=%1))").arg(mail);
